...
function AppContent() {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.contentContainer}>
      <View style={styles.header}><Text style={styles.brand}>PANSILU STREAM</Text><Text style={styles.subtitle}>Watch anywhere</Text></View>
      ...
      {mediaType !== 'torrent' && <View style={styles.providerGrid}>{getProviders().filter((provider) => provider.id !== 'torrent').map((provider) => <Pressable key={provider.id} onPress={() => { if (provider.id === 'agg') { Alert.alert('Provider warning', 'Titan may not have this movie or episode available.', [{ text: 'Cancel', style: 'cancel' }, { text: 'Continue', onPress: () => setProviderId(provider.id) }]); } else if (['pansilu', 'vidsrc', 'vidlink', 'embed2', 'cdn'].includes(provider.id)) { Alert.alert('Provider warning', `${provider.name} may redirect or show popups. Continue?`, [{ text: 'Cancel', style: 'cancel' }, { text: 'Continue', onPress: () => setProviderId(provider.id) }]); } else setProviderId(provider.id); }} style={[styles.provider, providerId === provider.id && styles.activeProvider]}><Text style={styles.providerText}>{provider.name}</Text></Pressable>)}</View>}
